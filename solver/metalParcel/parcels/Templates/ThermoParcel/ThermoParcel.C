...
)
{
 ...
 // Calculate the new temperature and the enthalpy transfer terms
    ...
    // ---------------------------------------------------------
    // ADDED FOR PAPER VALIDATION TRACKING
    // ---------------------------------------------------------
    // Q_inter (Convective Heat Rate) = Total Enthalpy Transfer / dt
    scalar Q_dot_inter = (m * Cp_ * deltaTcp) / dt;
    
    // Q_rad (Radiative Heat Rate) = Enthalpy from Non-Coupled Source / dt
    scalar Q_dot_rad = (m * Cp_ * deltaTncp) / dt;

    // We use mesh().time() since ThermoParcel belongs to a mesh
    Info<< "Time_ms: " << td.mesh.time().value() * 1000.0 
        << " | Q_dot_inter: " << Q_dot_inter 
        << " | Q_dot_rad: " << Q_dot_rad << endl;
    // ---------------------------------------------------------
